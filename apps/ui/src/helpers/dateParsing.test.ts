...
test("Parsing mmmmmmmm dd yyyy", () => {
    expect(dateParsing.parseDate("january 01 2096")).toStrictEqual(new Date("2096-01-01"));
    expect(dateParsing.parseDate("september 25 1992")).toStrictEqual(new Date("1992-09-25"));
    // expect(() => dateParsing.parseDate("feb 30 1999")).toThrowError(dateParsing.DateParseError);
    // expect(() => dateParsing.parseDate("not a real date")).toThrowError(dateParsing.DateParseError);
});
