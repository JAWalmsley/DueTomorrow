...
describe('User Database', function () {
    ...
    beforeEach(function () {
        return __awaiter(this, void 0, void 0, function () {
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0: return [4 /*yield*/, userDB.clearDB()];
                    case 1:
                        _a.sent();
                        return [2 /*return*/];
                }
    ...
    it('sets up table', function () {
        userDB.setUpTable().then(function () {
            testingCon.get("SELECT name FROM sqlite_master WHERE type='table' AND name='logins';", function (e, r) {
    ...
    it('creates a user', function (done) {
        userDB.setUpTable()
            .then(function () { return userDB.create(testData); })
            .then(function () { return testingCon.get("SELECT id, password FROM logins WHERE id = ?;", testData.id, function (e, r) {
    ...
    it('gets a user', function (done) {
        userDB.setUpTable()
            .then(function () { return userDB.create(testData); })
            .then(function () { return userDB.getByUserID(testData.id); })
    ...
    it('gets all users', function (done) {
        userDB.setUpTable()
            .then(function () { return userDB.create(testData); })
            .then(function () { return userDB.create(testData2); })
            .then(function () { return userDB.getAll(); })
            .then(function (response) {
            assert.notEqual(response, null);
            assert.equal(response.length, 2);
            assert.equal(response.some(function (item) { return item.id == testData.id; }), true);
            assert.equal(response.some(function (item) { return item.id == testData2.id; }), true);
            done();
        });
    });
    it('changes a user\'s password', function (done) {
        ...
        userDB.setUpTable()
            .then(function () { return userDB.create(testData); })
            .then(function () { return userDB.setPassword(testData.id, newpass); })
            .then(function () { return userDB.getByUserID(testData.id); })
            .then(function (response) {
            assert.equal(response.id, testData.id);
